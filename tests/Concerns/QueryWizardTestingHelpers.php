...
{
    protected function createEloquentWizardFromQuery(array $query = [], mixed $subject = null): EloquentQueryWizard
    {
        $subject = $subject ?? TestModel::class;
        $parameters = new QueryParametersManager(new Request($query));

        return new EloquentQueryWizard(
            is_string($subject) ? $subject::query() : $subject,
            $parameters
        );
    }
